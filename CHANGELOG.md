...
- New setting **"Paragraph re-sync strictness"** (2-6 words): the minimum number of consecutive words that must match before voice tracking is allowed to jump into another paragraph after a skipped phrase. Previously fixed at 3 words; now user-configurable.

### Fixed

- Removed an internal 4-word cap in the re-sync matcher that would have silently disabled paragraph jumps entirely for strictness settings above 4 words.
...
- Internal refactor: `App.jsx` split from a 4567-line monolith into hooks (`useScriptLibrary`, `useSettings`, `useWebMCP`, `useAutoScroll`, `useSpeechRecognition`, `useKeyboardShortcuts`) and presentational components (`Toolbar`, `TeleprompterDisplay`, `SettingsPanel`, `ScriptList`, script/reset/shortcuts modals). No behavior change; verified with the full test suite plus manual voice-tracking and panel/modal smoke tests.
...
- 2.4.4 bounded normal token-advance matching (`tryAdvanceByTokens`) to the current paragraph only, to stop it latching onto a coincidental repeated word/phrase in the next paragraph. That was too strict: every real paragraph-to-paragraph transition then had to go through the much stricter `findResyncMatch` fallback (3-gram, 2 exact matches), which often couldn't find a strong enough anchor right after a pause, and the tracker got stuck at paragraph boundaries. Both the fast path and the resync fallback now search up to `PARAGRAPH_LOOKAHEAD` (2) paragraphs ahead of the current one, so normal reading flows across paragraph breaks while a match many paragraphs away still can't win.
